...
/**
 * The single place the paused state changes, so the note can never disagree
 * with it.
 */
function setFocusPaused(paused) {
  if (state.focusPaused === paused) return;
  state.focusPaused = paused;
  timerNote.textContent = paused ? 'Paused \u00b7 resumes when you return' : 'Pauses if you leave';
}
...
function frame(now) {
  ...
  // If frames are arriving at a normal cadence then the app is plainly being
  // drawn, so it is not away - whatever the platform did or did not tell us.
  //
  // This exists because focusPaused used to be set and cleared only by
  // visibilitychange, and Android's WebView is not reliable about firing it:
  // WebView.onPause makes no promise about the page's visibility state, and
  // pauseTimers stops JavaScript immediately afterwards. The hidden side of
  // the event arrived, the visible side on return did not, and the session sat
  // frozen with the only available control being the one that cancels it.
  //
  // A backgrounded or sleeping app issues no animation frames at all, so
  // coming back produces one long gap and then ordinary ones. That is a fact
  // about the clock rather than a platform courtesy, which is the point: it
  // holds in a plain browser and in the WebView alike. visibilitychange is
  // still handled, because where it works it responds a frame sooner - it is
  // just no longer the only way out.
  if (state.focusPaused && gap <= 0.25) setFocusPaused(false);
